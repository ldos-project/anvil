...
auto scoring_fn = [](const vulcan::feature_store& fs, int64_t obj_id) -> double {
    ...
    float w_1s  = fs.get_ewma(f_accesses, obj_id, ALPHA_1S);  // reconstructed EWMA for 1-second horizon
    float w_10s = fs.get_ewma(f_accesses, obj_id, ALPHA_10S);  // reconstructed EWMA for 10-second horizon
